...
extern int __fraise_bootloader_start__, __fraise_bootloader_length__;
#if 0 // normal: boot to app
#define FLASH_ADDR_MIN ((uint32_t)&__fraise_app_start__)
#define FLASH_ADDR_MAX (FLASH_ADDR_MIN + (uint32_t)&__fraise_app_length__)
#else // debug: boot to fraise_bootloader
#define FLASH_ADDR_MIN ((uint32_t)&__fraise_bootloader_start__)
#define FLASH_ADDR_MAX (FLASH_ADDR_MIN + (uint32_t)&__fraise_bootloader_length__)
#endif
...
void processLine() {
	if(lineBuf[0] == '#') processSysLine();
		...
		printf("l usb_bld jump to 0x%08lX\n", FLASH_ADDR_MIN);
