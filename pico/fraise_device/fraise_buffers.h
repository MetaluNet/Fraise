 ...
 */
#ifndef _FRAISE_BUFFER_H
#define _FRAISE_BUFFER_H
...
void    txbuf_write_finish(bool isChar);    // Validate the message.
...
void    rxbuf_write_finish(bool isChar, bool isBroadcast); // Validate the message.

uint8_t rxbuf_read_init();                  // Start reading the next available message. Returns the length of the message (0 if none), 
                                            // plus 64 if the message is a broadcast one, plus 128 if it's a "char" one.
