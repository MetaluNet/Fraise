...
void WriteBlock()
{
	...
	if(address.UL == APP_START) {
		...
		mem[3] = (unsigned char)(0xf0 + (((BLD_START / 2) / 0xffff) & 0x0f));
		//mem[3] = 0xf0 + ((BLD_START >> 17) & 0x0f);
		Flags.WR_APP_BB = 1;
	}
...
#define SeekBuf(index) bufp = buf + index
...
unsigned char GetHex2(unsigned char i)
{
	tmp1 = buf[i] - '0';
	if(tmp1 > 9) tmp1 += '9' - 'A' + 1;
	tmp2 = buf[i + 1] - '0';
...
unsigned char GetHex3(unsigned char h, unsigned char l)
{
	h = h - '0';
	if(h > 9) h += '9' - 'A' + 1;
	l = l - '0';
...
void HexPacket()
{
	...
	cs += (rectype = GetHex());
	/*PrintLetterNL('0' + GetHex2(2));
	PrintLetterNL('0' + GetHex2(6));*/
	//PrintLetterNL(buf[2]);
	//PrintLetterNL(buf[3]);
	//PrintLetterNL('0' + GetHex3(buf[2], buf[3]));
	/*PrintLetterNL('0' + GetHex2(2));
	PrintLetterNL('0' + GetHex2(6));
	PrintLetterNL('0' + reclen);
	PrintLetterNL('0' + rectype);*/
	if(rectype == 4) {
		...
		if(buf[0]) {
			...
			if(Flags.VERIFIED) {
				if(c == 'E') Echo();
					...
					//putchar(' ');
					putchar('v');
					putchar('0' + Flags.VERBOSE1 + 2*Flags.VERBOSE2);
					putchar('\n');
				}
				else if (c == 'd') {
					Flags.DONT_WRITE = (buf[2] != '0');
				}
			}
		}
