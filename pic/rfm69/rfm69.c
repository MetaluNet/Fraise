...
static volatile uint8_t PAYLOADLEN = 0;
...
char RFM69init(char isRFM69HW, uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  ...
  const uint8_t CONFIG[][2] =
  {
    ...
    /* 0x03 */ { REG_BITRATEMSB, /*RF_BITRATEMSB_55555*/RF_BITRATEMSB_200KBPS}, // default: 4.8 KBPS
    /* 0x04 */ { REG_BITRATELSB, /*RF_BITRATELSB_55555*/RF_BITRATELSB_200KBPS},
    /* 0x05 */ { REG_FDEVMSB, RF_FDEVMSB_200000}, // RF_FDEVMSB_50000 ; default: 5KHz, (FDEV + BitRate / 2 <= 500KHz)
    /* 0x06 */ { REG_FDEVLSB, RF_FDEVLSB_200000},
...
// for 200k:  
	/* 0x19 */ { REG_RXBW, RF_RXBW_DCCFREQ_010 | RF_RXBW_MANT_16 | RF_RXBW_EXP_1 }, // (BitRate < 2 * RxBw)
...
uint32_t txStart, txTime1, txTime2, txTime3;
...
void sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, char requestACK, char sendACK)
{
  ...
  txTime1 = time()-txStart; txStart = time();
  ...
  txTime2 = time()-txStart; txStart = time();
  
  while (digitalRead(RF69_IRQ_PIN) == 0 && time() - txStart < (RF69_TX_LIMIT_MS*1000UL)); // wait for DIO0 to turn HIGH signalling transmission finish
  //while (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PACKETSENT == 0x00); // wait for ModeReady
  setMode(RF69_MODE_STANDBY);
  
  txTime3 = time()-txStart;
}
...
//{
  /*if (_haveData) {
	...
	/*int i;
	for(i = 0; i < 16 ; i++) tmp = digitalRead(RF69_IRQ_PIN);
  return false;*/
  //if(digitalRead(RF69_IRQ_PIN)) Nop();
  tmp = digitalRead(RF69_IRQ_PIN);
  if(tmp != irq_pin)
  {
    //Nop();
    irq_pin = tmp;
    if(irq_pin) interruptHandler();
  }
