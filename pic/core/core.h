...
#define ANALOG_WRITE_(pwm, val) do{ \
		CCP##pwm##CONbits.DC##pwm##B1 = val & 2;\
		CCP##pwm##CONbits.DC##pwm##B0 = val & 1;\
		CCPR##pwm##L = val >> 2;\
	} while(0)
...
#define	microToTime(T) (((unsigned long)T * 1000) / (FOSC / 64000UL)) //microseconds to time cycles
#define	timeToMicro(T) (((unsigned long)T * (FOSC/64000UL)) / 1000) //time cycles to microseconds

/** \name Time functions */
//@{
unsigned long int time(void); ///< Get time since bootup in 64/FOSC steps (1us @ 64MHz, 8us @ 8MHz).
