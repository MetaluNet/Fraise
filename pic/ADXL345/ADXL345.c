...
void ADXL345Set(ADXL345 *dev, byte reg, byte value)
{
...
//	writeTo(address, ADXL345_DATA_FORMAT, 0b00001000); // full range
	...
	writeTo(address, ADXL345_BW_RATE, ADXL345_BW_25);
	writeTo(address, ADXL345_FIFO_CTL, 129 /* stream mode */);
}
...
void ADXL345Service(ADXL345 *dev)
{
	...
	i2cm_begin(address, 0);
	i2cm_writechar(ADXL345_INT_SOURCE);
	i2cm_stop();
	...
	int_source = i2cm_readchar();
	
	if( (int_source & (1<<ADXL345_INT_WATERMARK_BIT)) != 0) {
		i2cm_ack();
		i2cm_readchar(); i2cm_ack(); // discard DATA_FORMAT byte
